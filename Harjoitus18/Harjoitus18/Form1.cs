...
namespace Harjoitus18
{
    public partial class Form1 : Form
    {
        ...
        private void oppilaitoksetCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            postinroLB.Text = oppilaitos.Rows[oppilaitoksetCB.SelectedIndex]["OPostinumero"].ToString();
            kaupunkiLB.Text = oppilaitos.Rows[oppilaitoksetCB.SelectedIndex]["OPostitoimipaikka"].ToString();
            puhoLB.Text = oppilaitos.Rows[oppilaitoksetCB.SelectedIndex]["OPuhelin"].ToString();

            var rivit = vastuuHenkilot.Select("OID = " + viite);

            if (rivit.Length > 0)
            {
                yhteys = rivit.CopyToDataTable();
                vastuuhenkilotCB.DataSource = yhteys;
                vastuuhenkilotCB.DisplayMember = "VNimi";
            }
            else
            {
                vastuuhenkilotCB.DataSource = null;
            }
            vastuuhenkilotCB.DataSource = yhteys;
            vastuuhenkilotCB.DisplayMember = "VNimi";
        }

        private void vastuuhenkilotCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            titteliLB.Text = yhteys.Rows[vastuuhenkilotCB.SelectedIndex]["VTitteli"].ToString();
            osastoLB.Text = yhteys.Rows[vastuuhenkilotCB.SelectedIndex]["VSijainti"].ToString();
            spostiLB.Text = yhteys.Rows[vastuuhenkilotCB.SelectedIndex]["VSahkoposti"].ToString();
            puhvLB.Text = yhteys.Rows[vastuuhenkilotCB.SelectedIndex]["VPuhelin"].ToString();
        }
